    ...
    return (
        <aside className="w-20 hover:w-64 bg-surface h-full flex flex-col justify-between py-6 z-50 overflow-hidden custom-scrollbar shadow-[10px_0_20px_#e2e6f0] transition-all duration-300 ease-in-out group absolute md:fixed">
                    ...
                    {navItems.map((item, index) => {
                        ...
                        return (
                            <button
                                ...
                                title={item.label}
                                className={'relative w-full h-12 rounded-[14px] flex items-center gap-4 transition-all duration-300 ' + (isActive ? 'bg-background shadow-inner' : 'bg-surface shadow-soft hover:shadow-card')}
                            >
                                <div className="w-12 h-12 shrink-0 flex items-center justify-center">
                                    <item.icon
                                        size={20}
                                        strokeWidth={isActive ? 2.5 : 2}
                                        className={isActive ? 'text-primary' : 'text-text-muted hover:text-primary'}
                                    />
                                </div>
                                <span className={'text-sm font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 ' + (isActive ? 'text-primary' : 'text-text-secondary')}>
                                    {item.label}
                                </span>
                                {isActive && (
            ...
            <div className="flex flex-col gap-5 w-full px-4 overflow-hidden">
                {/* Circular Daily Goal */}
                <div className="relative cursor-pointer w-full h-12 flex items-center bg-background rounded-[14px] shadow-inner" title="Daily Goal: 80%">
                    <div className="w-12 h-12 shrink-0 flex items-center justify-center relative">
                        <svg width="32" height="32" className="rotate-[-90deg]">
                            <circle cx="16" cy="16" r="12" fill="none" stroke="transparent" strokeWidth="3" />
                            <circle cx="16" cy="16" r="12" fill="none" stroke="#10B981" strokeWidth="3" strokeLinecap="round" strokeDasharray="75.4" strokeDashoffset="15.08" className="drop-shadow-md transition-all duration-1000 ease-out" />
                        </svg>
                    ...
                    <div className="flex flex-col items-start justify-center ml-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
                        <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Daily Goal</span>
